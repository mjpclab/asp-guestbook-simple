...
<form method="post" action="mdel.asp" onsubmit="return confirm('确实要删除选定留言吗？')">
	...
	<div class="admin-tools">
		...
		<a class="tool" href="reply.asp?id=<%=rs("id")%>&amp;page=<%=ipage%>" title="回复此留言"><img alt="" src="image/icon_reply.gif" />[回复]</a>
		<a class="tool" href="del.asp?id=<%=rs("id")%>&amp;page=<%=ipage%>" title="删除留言"><img alt="" src="image/icon_del.gif" />[删除]</a>
		<% if rs("replied")=true then %><a class="tool" href="delreply.asp?id=<%=rs("id")%>&amp;page=<%=ipage%>" title="删除回复"><img alt="" src="image/icon_delreply.gif" />[删除回复]</a> <% end if %>
	</div>
	<%
	rs.MoveNext
	wend
