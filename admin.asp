...
<form method="post" action="admin_mdel.asp" onsubmit="return confirm('确实要删除选定留言吗？')">
	...
	<div class="admin-tools">
		...
		<a class="tool" href="admin_reply.asp?id=<%=rs.Fields("id")%>&amp;page=<%=ipage%>" title="回复此留言"><img alt="" src="asset/image/icon_reply.gif" />[回复]</a>
		<a class="tool" href="admin_del.asp?id=<%=rs.Fields("id")%>&amp;page=<%=ipage%>" title="删除留言"><img alt="" src="asset/image/icon_del.gif" />[删除]</a>
		<% if rs.Fields("replied") then %><a class="tool" href="admin_delreply.asp?id=<%=rs.Fields("id")%>&amp;page=<%=ipage%>" title="删除回复"><img alt="" src="asset/image/icon_delreply.gif" />[删除回复]</a> <% end if %>
	</div>
	<%
	rs.MoveNext
	wend
